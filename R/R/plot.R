...
#' Plot ICC coefficient metrics.
#'
#' @param icc_scores A vector, the ICC coefficient metrics to be plotted as a histogram.
#' @return A ggplot object. This plot may be rendered by outputting it to the command line or modified using ggplot semantics.
#' @family plot
#' @export
plot_metrics_gaussian_icc_score <- function(icc_scores) {
  mean_icc_score <- mean(icc_scores)
  icc_score_label <- paste("Mean ICC Score = ", round(mean_icc_score, digits = 3), sep = "")
  df <- data.frame(icc_scores = icc_scores, stringsAsFactors = FALSE)
  
  g <- 
    ggplot2::ggplot(df, ggplot2::aes(x = icc_scores)) +
    ggplot2::geom_histogram(binwidth = 0.02) + 
    ggplot2::geom_vline(xintercept = mean_icc_score, linetype = "dotted") + 
    ggplot2::annotate("text", label = icc_score_label, x = 0.2, y = 0.2, size = 8) + 
    ggplot2::scale_x_continuous("ICC Score", limits = c(0, 1)) + 
    ggplot2::scale_y_continuous("Frequency", label = scales::comma) + 
    ggplot2::ggtitle("Distribution of Correlation Scores") + 
    ggplot2::theme_bw()
